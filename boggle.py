...
class Boggle():
    ...
    def read_dict(self, dict_path):
        ...
        words = [w.strip() for w in dict_file] #strips the words of any spacing from the words in dict_file
        dict_file.close() #closes the file
        return words #returns words with no spacing from the dict_file
    ...
    def make_board(self):
        ...
        for y in range(5): #y values from 0 to 4 not including 5
            row = [choice(string.ascii_uppercase) for x in range(5)] # creates a row up ascii uppercase letters of 5
            board.append(row) #add rows to the board

        return board # return board with rows and columns of 5
    ...
    def find(self, board, word):
        ...
        for y in range(0, 5): # is the 0 needed?
            for x in range(0, 5): # is the 0 needed?
